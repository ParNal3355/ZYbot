...
import org.example.zybot.begin.Modules.MCremake.JE.JEServeInformation;
...
public class JEMain implements EventStrategy {
    ...
    private final JEServeInformation jeServeInformation;

    @Autowired
    public JEMain(Container container, GetAllServices getAllServices, GainIPofAlias gainIPofAlias
    , BindingServices bindingServices, DeleteAlias deleteAlias, JEServeInformation jeServeInformation,readMCHelp readMCHelp) {
        ...
        this.jeServeInformation = jeServeInformation;
    }
    ...
    public RPerson process(Person person) {
        ...
        if (!message[0].equals("JE")) {
            rPerson.setText("-1");
            return rPerson;
        }

        if (message.length ==1) {
            rPerson.setText("参数不足，我不知道要做什么奥...是不知道指令吗？可以使用 /JE 帮助 指令查询奥~");
            return rPerson;
        }

        switch (message[1]) {
            case "查看全部" -> {
                rPerson.setText(getAllServices.getIpsAndPortsByBid(person.getBid(), 2));
                return rPerson;
            }
            case "帮助", "help" -> {
                rPerson.setText(readMCHelp.readFile(2));
                return rPerson;
            }
            case "绑定" -> {
                if (message.length < 4) {
                    rPerson.setText("参数不足...可使用 /JE help 指令查看帮助奥~");
                    return rPerson;
                }
                rPerson.setText(bindingServices.bindServer(person.getBid(), message[2], message[3], 2));
                return rPerson;
            }
        }

        //确保参数足够：/JE 指令名 内容
        if (message.length < 3) {//参数不足 只有/JE xxx
            rPerson.setText("参数不足...可使用 /JE help 指令查看帮助奥~");
            return rPerson;
        }
        ...
        if (message[1].equals("删除")) {
            ...
            rPerson.setText(deleteAlias.deleteByName(person.getBid(),message[2],2));
            return rPerson;
        }
        ...
        if (message[1].equals("服务器状态")){
            if (found1){//输入的是地址
                rPerson.setText(jeServeInformation.handle(message[2]));
                return rPerson;
            }else {//输入的是别名
                MCPair<String,Integer> a= gainIPofAlias.madeIP(person.getBid(),message[2],2);
                if (a.getSecond()==null){
                ...
                rPerson.setText(jeServeInformation.handle(add));
                return rPerson;
            }
        }else {
            rPerson.setText("该指令我不认识奥，是输入错误了吗？可以使用/JE help 指令查看帮助奥~");
            return rPerson;
        }
    }
