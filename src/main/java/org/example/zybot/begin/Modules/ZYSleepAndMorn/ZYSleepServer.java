...
import org.example.zybot.begin.Modules.ZYSleepAndMorn.body.SleepServer;
...
//晚安指令的策略类
public class ZYSleepServer implements EventStrategy {
    private final Container container;
    private final SleepServer sleepServer;

    @Autowired
    public ZYSleepServer(Container container, SleepServer sleepServer) {
    ...
    public RPerson process(Person person) {
        ...
        if (!message[0].equals("晚安")) {
            rPerson.setText("-1");
            return rPerson;
        }

        rPerson.setText(sleepServer.processSleep(person.getBid(), person.getUid()));
        return rPerson;
    }
